...
def index_files(files):
    for file in files:
        ...
        with open('temp/{}.index'.format(tools.process(file)), 'wb') as dump: pickle.dump(index_keywords(text.split()), dump)
...
def invert_index(files):
    ...
    for file in files:
        print(file)
        with open('temp/{}.index'.format(tools.process(file)), 'rb') as dump: index = pickle.load(dump)
